...
class DatabaseSettings(BaseSettings):
    ...
    POSTGRES_TEST_DB_NAME: Annotated[str, Field(validation_alias="POSTGRES_TEST_DB_NAME")]
    ...
    DB_POOL_TIMEOUT: float = Field(
        default=30.0, description="Seconds to wait for a connection from the pool.", gt=0
    )
    ...
    DB_USE_TWOPHASE: bool = Field(
        default=False, description="Enable two-phase commits for distributed transactions."
    )
